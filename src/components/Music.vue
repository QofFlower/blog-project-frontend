<template>
  <div id="app">
    <!-- 浏览器原生播放器 -->
    <div v-show="false">
      <audio ref="audioUrlRefText" :src="song.url" id="audio" controls></audio>
    </div>
    ...
    <div id="music-container" class="audioplayer">
      <div class="ap-status">
            ...
            <div class="ap-progress-line">
              <div class="ap-play-track" ref="track">
                ...
                <div class="ap-play-bar-wrap">
                  ...
                  <div
                    class="ap-buffer-bar"
                    :style="{ transform: 'scaleX(' + bufferedScaleX + ')' }"
                  ></div>
                  <!-- 当前进度 -->
                  <div
        ...
        <!-- 提示区 -->
        <div class="ap-toast" :class="{ 'ap-toast-hide': error.length <= 0 }">
          {{ error }}
        </div>
      </div>
    </div>
  ...
  created() {
    ...
    this.$axios({
      ...
      if (res) {
        ...
        that.songList = res.data.data;
        that.song = that.songList[0];
      }
    });
  },
};
